...
class DnsService
{
    ...
    {
        $output = '';
        $users = DB::connection('openpanel')->table('user')->where('id', '!=', '')->get();

        foreach ($users as $user) {
            $dnsEmail = self::dnsEmail($user->email ?? '');
            self::addZoneForce($user->domain, $user->ip_address, $dnsEmail, 'none');

            $domains = DB::connection('openpanel')->table('domains')->where('user', $user->username)->get();
    ...
    {
        $users = DB::connection('openpanel')->table('user')->where('username', $username)->get();
...
';
    ...
    {
        try {
            $row = DB::connection('openpanel')->table('nameserver')->first();
            if ($row) {
                return [
    ...
    {
        try {
            DB::connection('openpanel')->table('nameserver')->update([
                'ns1_name' => $ns1,
                'ns2_name' => $ns2,
                'ns1_ip' => $ns1Ip,
                'ns2_ip' => $ns2Ip,
    ...
    public static function dnsManagerPushAll(): ?string
    {
        ...
        $domainArray = [];
        $users = DB::connection('openpanel')->table('user')->where('id', '!=', '')->orderBy('username')->get();
        foreach ($users as $user) {
            $domains = DB::connection('openpanel')->table('domains')->where('user', $user->username)->get();
            foreach ($domains as $d) {
