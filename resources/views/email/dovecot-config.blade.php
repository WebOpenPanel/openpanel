@extends('layouts.app')
@section('title', 'Dovecot Configuration')
@section('content')
<div class="space-y-4">
    <div class="flex items-center gap-3 mb-2 flex-wrap">
        ...
        <a href="{{ route('email.dovecot-config') }}" class="px-3 py-1.5 text-sm font-medium bg-indigo-100 text-indigo-700 rounded-lg">Dovecot</a>
    </div>
    <div class="bg-white rounded-xl shadow-sm border p-5">
        <h3 class="text-sm font-semibold text-gray-700 mb-3"><i class="fas fa-cog mr-2 text-indigo-500"></i>dovecot.conf</h3>
        <form method="POST" action="{{ route('email.save-dovecot-config') }}" class="space-y-3">
            @csrf
